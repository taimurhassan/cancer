...
def identity_block(input_tensor, kernel_size, filters, stage, block):
    ...
    x = Conv2D(filters1, (1, 1), data_format=IMAGE_ORDERING, dilation_rate=(2, 2), 
               name=conv_name_base + '2a')(input_tensor)
    x = BatchNormalization(axis=bn_axis, name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)

    x = Conv2D(filters2, kernel_size, data_format=IMAGE_ORDERING, dilation_rate=(3, 3), 
               padding='same', name=conv_name_base + '2b')(x)
    x = BatchNormalization(axis=bn_axis, name=bn_name_base + '2b')(x)
    x = Activation('relu')(x)

    x = Conv2D(filters3, (1, 1), data_format=IMAGE_ORDERING, dilation_rate=(4, 4), 
...
def conv_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = Conv2D(filters2, kernel_size, data_format=IMAGE_ORDERING, dilation_rate=(2, 2), 
               padding='same', name=conv_name_base + '2b')(x)
    x = BatchNormalization(axis=bn_axis, name=bn_name_base + '2b')(x)
    x = Activation('relu')(x)

    x = Conv2D(filters3, (1, 1), data_format=IMAGE_ORDERING, dilation_rate=(3, 3), 
               name=conv_name_base + '2c')(x)
    x = BatchNormalization(axis=bn_axis, name=bn_name_base + '2c')(x)
