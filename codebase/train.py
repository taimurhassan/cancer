...
def customLoss(yTrue,yPred):
    ...
    a2 = 0.2
    a3 = 0.6
    ce = keras.losses.categorical_crossentropy(yTrue,yPred)
    ft = K.mean(1-( (K.sum(yTrue * yPred)/(K.sum(yTrue*yPred + beta1 * (1-yTrue)*yPred + beta2 * yTrue* (1-yPred)) ) ) ) )**(1/gamma)
    dice = K.mean(1-(2 * K.sum(yTrue * yPred))/(K.sum(yTrue + yPred)))
    hybrid = a1 * keras.losses.categorical_crossentropy(yTrue,yPred) + a2 * K.mean(dice) + a3 * K.mean(ft)
    return hybrid #K.mean(ft) #K.mean(dice + keras.losses.categorical_crossentropy(yTrue,yPred))
...
def hLoss(y_true, y_pred):
    ...
    def focal_tversky_loss(y_true, y_pred):
        ...
        FP = tf.reduce_sum(y_pred*Gcnot, axis=(1,2,3));
        FN = tf.reduce_sum(Pcnot*y_true, axis=(1,2,3));

        numer = TP + eps;
        denom = TP + (beta1*FP) + (beta2*FN) + eps;
        
        lossTIc = (1 - numer/denom) ** gammaI;
        lossFTI = tf.reduce_sum(lossTIc, axis=(1,2,3));
        
        return tf.reshape(lossFTI, (-1, 1, 1))
    alpha1 = 0.2
    alpha2 = 0.2
    alpha3 = 0.6
    return keras_backend.mean((alpha1 * keras.losses.categorical_crossentropy(y_true, y_pred)) + (alpha2 * dice_loss(y_true, y_pred)) + (alpha3 * focal_tversky_loss(y_true, y_pred)))
...
#loss="categorical_crossentropy",#
    ...
    if validate:
        val_gen = ImageSegmentationGen(
            ...
            n_classes, input_height, input_width, output_height, output_width , do_augment=do_augment)
